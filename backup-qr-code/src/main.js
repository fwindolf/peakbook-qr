...
class PeakbookQRApp {
  ...
  setupRealTimeValidation() {
    ...
    // Caption validation
    this.captionInput?.addEventListener(
      'input',
      this.debounce(() => {
  ...
  updateCharCount() {
    ...
    // Update help text
    if (this.tokenHelp) {
      this.tokenHelp.textContent = this.validator.getTokenHelpText(
  ...
  async generateSticker() {
    try {
      ...
      if (!validation.valid) {
        ...
        this.utils.debug('Validation failed', {
          firstError,
      ...
      // Generate QR code
      const qrResult = await this.qrGenerator.generateFromToken(
        formData.token,
        {
      ...
      this.utils.debug('SVG sticker created', {
        hasSticker: !!stickerSvg,
        tagName: stickerSvg?.tagName,
      });
  ...
  displayPreview(stickerSvg) {
    ...
    this.previewContainer.className =
      'border-2 border-solid border-gray-200 rounded-lg p-4 min-h-[400px] flex items-center justify-center bg-white';
    ...
    // Create preview SVG
    const previewSvg = this.svgBuilder.createPreviewSVG(
      stickerSvg,
      this.config.UI.PREVIEW_SIZE_PX
    );
    previewSvg.setAttribute(
      'class',
      'sticker-preview cursor-zoom-in max-w-full h-auto'
    );
        ...
        if (preferences.colors) {
          this.bgColorInput &&
            (this.bgColorInput.value =
              preferences.colors.BACKGROUND || this.config.COLORS.BACKGROUND);
          this.dotsColorInput &&
            (this.dotsColorInput.value =
              preferences.colors.DOTS || this.config.COLORS.DOTS);
          this.cornersColorInput &&
            (this.cornersColorInput.value =
              preferences.colors.CORNERS || this.config.COLORS.CORNERS);
        }
  ...
  saveToHistory(formData) {
    try {
      const history = JSON.parse(
        localStorage.getItem('peakbook-qr-history') || '[]'
      );
