...
export class Validator {
    ...
    validateParameterName(paramName) {
        ...
        if (!validParamPattern.test(paramName)) {
            return {
                valid: false,
                error: 'Parameter name must start with a letter and contain only letters, numbers, underscores, or hyphens'
            };
        }
    ...
    formatTokenInput(input) {
        return this.sanitizeInput(input, 'token').substring(0, this.config.TOKEN_LENGTH);
    }
